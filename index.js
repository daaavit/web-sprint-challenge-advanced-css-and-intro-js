...
function get20s(orginalArtistsArray) {
  let artists = [];

  for (let i = 0; i < orginalArtistsArray.length; i++) {
    let artist = orginalArtistsArray[i];

    let artistYears = artist.years;

    let splitedYears = artistYears.split('-');

    let bornYear = splitedYears[0];
    let diedYear = splitedYears[1];

    if (bornYear >= 1900 && diedYear <= 2000) {
...
function removeArtist(array, number) {
  ...
  const index = array.indexOf(artist);
...
function addArtist(array) {
  const artist =
  {
    ...
    bio: "lorem use lorem ipsum lorem ubsun shmibsun da"
  }
  array.push(artist);
...
console.log("addArtist", addArtist(artists));
